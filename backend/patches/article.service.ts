...
const buildFindAllQuery = (query: any, username: string | undefined) => {
  ...
  if (orAuthorQuery.length) {
    authorFilter.OR = orAuthorQuery;
  }
  if (andAuthorQuery.length) {
    authorFilter.AND = andAuthorQuery;
  }
  if (Object.keys(authorFilter).length) {
    queries.push({ author: authorFilter });
  }
...
export const getArticles = async (query: any, username?: string) => {
  ...
  const where = andQueries.length ? { AND: andQueries } : undefined;
...
export const createArticle = async (article: any, username: string) => {
  if (!article) {
    throw new HttpException(422, { errors: { article: ["can't be blank"] } });
  }
  ...
  return {
    ...createdArticle,
    tagList: createdArticle.tagList.map(tag => tag.name).sort(),
...
export const updateArticle = async (article: any, slug: string, username: string) => {
  ...
  if (existing.author.username !== username) {
...
export const getCommentsByArticle = async (slug: string, username?: string) => {
  ...
  const comments = await prisma.article.findUnique({
    ...
    include: {
      comments: {
        where: queries.length ? { OR: queries } : undefined,
        select: {
          id: true,
          createdAt: true,
          updatedAt: true,
...
export const deleteComment = async (id: number, username: string, slug?: string) => {
  if (slug) {
    const article = await prisma.article.findUnique({
      where: { slug },
      select: { id: true },
